...
(defn- apply-date-separator [format-list date-separator]
  (if date-separator
    (for [fmt format-list]
      (if (sequential? fmt)
        [(str/replace (first fmt) #"/" date-separator)]
        fmt))
    format-list))
...
(defn- normalize-date-format [{:keys [date-format] :as options}]
  (if-let [new-opts (when (string? date-format) (get constants/known-datetime-styles date-format))]
    (merge options new-opts)
    options))
...
(defn- date-format-for-options
  ...
  "
  [{:keys [date-separator] :as options}]
...
;;; --------------------------------------------- Time formatters ----------------------------------------------------
(defn- english-time [inner]
  (concat [:hour-12-d [":"] :minute-dd]
          inner
          [[" "] :am-pm]))

(defn- english-time-seconds [inner]
  (english-time (concat [[":"] :second-dd] inner)))
...
(def ^:private time-style-to-format
  {"h:mm A" {nil            (english-time-seconds [])
   ...
   "HH:mm"  {nil            iso-time-seconds
             ...
             "milliseconds" (concat iso-time-seconds [["."] :millisecond-ddd])}})

(defn- time-format-for-options
  "The time format is resolved as follows:
  0. If a `:time-format` is provided directly as a [[builder]] structure, use that.
  1. If a `:time-format` is provided as a string, look it up in [[constants/known-time-styles]], throwing if not found.
  2. Check [[time-style-to-format]] for a supported `:time-style + :time-enabled` resolution pair.
  3. Look up `:time-style` in [[constants/known-time-styles]].
  4. Throw an exception, since the time style is unknown."
  [{:keys [time-enabled time-format time-style] :as options}]
  (or (when (string? time-format)
      ...
      (throw (ex-info "Unknown time style" options))))
