...
(defn ^:export format-for-parameter
  ...
  (let [options (options/prepare-options options)
        t       (shared.ut/parse-timestamp value options)]
    (if (not (shared.ut/valid-datetime? t))
...
(defn ^:export format-datetime-with-unit
  ...
  [value options]
  (let [{:keys [is-exclude no-range type unit] :as options} (options/prepare-options options)
        t       (shared.ut/parse-timestamp value options)]
    (cond
...
;; TODO The TS code should check isDateWithoutTime, and suppress time_enabled in that case.
