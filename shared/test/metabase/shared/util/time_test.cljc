...
(deftest parse-timestamp-test
  (testing "numbers are parsed into datetimes based on the unit"
    (with-redefs [shared.ut/now (fn [] (from test-epoch))]
      (are [exp-str input unit] (same? (from exp-str) (shared.ut/parse-timestamp input {:unit unit}))
  ...
  (testing "numbers with no unit are parsed as year numbers"
    (are [exp-str input] (same? (from-zulu exp-str) (shared.ut/parse-timestamp input {}))
  ...
  (testing "strings"
    (testing "with unit=day-of-week get parsed as eg. Mon"
      (with-redefs [shared.ut/now (fn [] (from test-epoch))]
        (are [exp-str input] (same? (from exp-str) (shared.ut/parse-timestamp input {:unit "day-of-week"}))
    ...
    (testing "with unit != day-of-week"
      (testing "and a time offset are parsed in that offset"
        (are [exp-str input] (same-instant? (from-zulu exp-str) (shared.ut/parse-timestamp input {}))
      ...
      (testing "and no time offset are assumed to be UTC"
        (is (same? (from-zulu "2022-12-14T13:37:45Z")
                   (shared.ut/parse-timestamp "2022-12-14T13:37:45" {}))))))

  (testing "existing date-time values are simply returned"
    (are [value] (let [t (shared.ut/parse-timestamp value)] (same? t (shared.ut/parse-timestamp t)))
      "2022-12-12T00:00:00"
...
(deftest same-day-month-year-test
  (let [ref-date      (from "2022-12-19T08:12:45")  ; Base
    ...
    (testing "same-day?"
      ...
      (is (not (shared.ut/same-day? same-day same-month)))
      (is (not (shared.ut/same-day? same-day same-year)))
      (is (not (shared.ut/same-day? same-day previous-year))))
    (testing "same-month?"
...
(deftest to-range-test
  (doseq [[exp-from exp-to date unit]
          ...
          :let [[from to] (shared.ut/to-range (shared.ut/parse-timestamp date nil)
                                              {:unit   unit
...
(deftest parse-time-test
  (testing "parsing time strings"
    (are [exp input] (same? (time-from exp) (shared.ut/parse-time input))
  ...
  (testing "Moment and LocalTime values are simply returned"
    (let [t (time-from "09:29")]
      (is (= t (shared.ut/parse-time t)))))

  (testing "numbers are treated as Unix timestamps"
    (is (thrown-with-msg? #?(:clj Exception :cljs js/Error)
                          #"Unknown input to parse-time; expecting a string"
                          (shared.ut/parse-time 12)))))
