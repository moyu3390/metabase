...
(s/defn ^:private col-info-for-field-clause*
  ...
  (let [join                      (when (:join-alias opts)
    ...
    (cond-> {:field_ref (mbql.u/remove-namespaced-options clause)}
      ...
      ;; return the same `:join-alias/original` if present -- the on used in the original query rather than whatever we
      ;; may have changed it to in the [[metabase.query-processor.middleware.escape-join-aliases]] middleware. See
      ;; #27464
      (or (:join-alias/original opts)
          (:join-alias opts)
          (:alias join))
      (assoc :source_alias (or (:join-alias/original opts)
                               (:join-alias opts)
                               (:alias join)))

      (:join-alias/original opts)
      (update :field_ref mbql.u/assoc-field-options :join-alias (:join-alias/original opts))
